...
const StatusIcon: React.FC<{ status: WorkflowStatus }> = ({ status }) => {
  ...
  if (status === 'active') {
    return (
      <div className="w-7 h-7 rounded-full border-2 border-[#1e3a6e] flex items-center justify-center flex-shrink-0">
        <div className="w-2.5 h-2.5 rounded-full bg-[#1e3a6e] animate-pulse" />
      </div>
    );
  }
  return (
...
const WorkflowStepItem: React.FC<WorkflowStepItemProps> = ({ step, isLast }) => (
  <div className="flex gap-3">
    <div className="flex flex-col items-center">
      ...
      {!isLast && (
        <div className={`w-0.5 flex-1 mt-1 ${step.status === 'completed' ? 'bg-[#00b894]' : 'bg-gray-200'}`} style={{ minHeight: 28 }} />
      )}
    </div>
    <div className="pb-5">
      <p className={`text-sm font-medium leading-7 ${step.status === 'pending' ? 'text-gray-400' : 'text-gray-800'}`}>
        {step.label}
      </p>
      {step.subLabel && (
        <p className="text-xs text-[#1e3a6e] font-medium -mt-1">{step.subLabel}</p>
      )}
    </div>
  </div>
);
